...
int main(int argc, const char *argv[])
{
	...
	// 2 blocks (16 words)
	// Bobcat hash = 0x2bce98fa6186
	len = 1;// number of 128-bit blocks
  //********************************************************************
	//********************************************************************
	//********************************************************************
    ...
    map< vector<int>, vector<int> > dict; // hash value -> message
    ...
    while(1){
      ...
      int v2[] = {abc[0], abc[1], abc[2]};
      int message2[BLEN];


  		getRandomMessage(message2);
      int y[BLEN];
      for (int k = 0; k < BLEN; k++) y[k] = message2[k];
      bobcat(v2, &y[8*i]);
      if (count  % 100000 == 0) {
        printArray(y);
      }

      vector<int> vecy(y, y + BLEN);
      vector<int> vecMessage(message2, message2 + BLEN);
      if (dict.count(vecy) > 0) {
        printf("message1: \n");
        printVec(dict[vecy]);
        printf("hash value of message1\n");
        printVec(vecy);

        printf("message2: \n");
        printArray(message2);
        printf("hash value of message2\n");
        printArray(y);
        break;
      }
      dict[vecy] = vecMessage;
